...
function convertirObjeto(texto) {
    ...
    for (let atributo of atributos) {
        ...
        mapa.set(
            atributo[0],
            Number(atributo[1]) ? Number(atributo[1]) : atributo[1]
        );
    }
...
function obtenerElementos(desc) {
    return inventario.filter((elemento) => elemento.descripcion.includes(desc));
}
// obtenerElementos = (desc) => inventario.filter((elemento) => elemento.descripcion.includes(desc));
